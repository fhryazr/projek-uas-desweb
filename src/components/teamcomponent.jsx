...
const TeamMember = ({ name, title, imageSrc, isMiddle }) => {
    ...
    const containerMarginBottom = isMiddle ? 'mb-4 md:mb-6 lg:mb-8' : 'mb-2 md:mb-4 lg:mb-6';
  
    return (
      <div className={`${containerPadding} ${containerMarginBottom} text-center transition-all duration-300`}>
        <div className={`${imageSizeClass} mx-auto`}>
          <img className={`rounded-full border-4 ${isMiddle ? 'border-yellow-300' : 'border-purple-300'}`} src={imageSrc} alt={name} />
        </div>
        <div className="text-xs md:text-sm lg:text-base font-semibold">{name}</div>
...
const TeamComponent = () => {
  ...
  const sliderStyle = {
    ...
    transform: `translateX(-${activeSetIndex * 100 / teamMemberSets.length}%)`,
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="overflow-hidden">
          <div className="flex" style={sliderStyle}>
            {teamMemberSets.map((set, setIndex) => (
              <div key={setIndex} className="flex justify-center space-x-8" style={{ width: '100%' }}>
        ...
        <div className="flex justify-center space-x-2 mt-4">
          {teamMemberSets.map((_, index) => (
            <button
              key={index}
              className={`h-2 w-2 rounded-full ${index === activeSetIndex ? 'bg-black' : 'bg-gray-300'}`}
